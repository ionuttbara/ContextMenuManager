...
namespace BluePointLilac.Controls
{
    public class MyListBox : Panel
    {
        public MyListBox()
        {
			...
			Color accentColor = ThemeManager.GetSystemAccentColor();

			// Using a slightly deeper dark for the main content area (32, 32, 32)
			this.BackColor = isDark ? Color.FromArgb(32, 32, 32) : Color.White;
    ...
    public class MyList : FlowLayoutPanel
    {
		...
		public MyListItem HoveredItem
		{
			...
			set
			{
				...
				if (hoveredItem != null)
				{
					// Restore to normal text color based on theme
					hoveredItem.ForeColor = isDark ? Color.White : Color.FromArgb(80, 80, 80);
				}
				hoveredItem = value;
				if (hoveredItem != null)
				{
					// Set highlighted text color (using Accent Color looks great in dark mode)
					value.ForeColor = isDark ? ThemeManager.GetSystemAccentColor() : Color.FromArgb(0, 138, 217);
					value.Focus();
				}
    ...
    public class MyListItem : Panel
    {
		public MyListItem()
		{
			...
			lblSeparator.BackColor = isDark ? Color.FromArgb(80, 80, 80) : Color.FromArgb(220, 220, 220);
			// ---------------------
